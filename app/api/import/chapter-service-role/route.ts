...
export async function POST(request: NextRequest) {
  try {
    ...
    const { storyId, chapter } = await request.json();
    
    // Basic validation
    if (!storyId || !chapter) {
      return NextResponse.json(
        { success: false, error: 'Missing storyId or chapter data' },
        { status: 400 }
      );
    }
    
    console.log('API: Importing chapter with service role');
    ...
    return NextResponse.json({
      success: true,
      chapterId,
      message: 'Chapter imported successfully using service role'
    });
